...
async function main() {
    ...
    try {
        ...
        let historyAfterTakeover = [];
        let hasHostChangedEvent = false;
        for (let attempt = 0; attempt < 20; attempt++) {
            historyAfterTakeover = await page2.evaluate(() => window.presence.history || []);
            hasHostChangedEvent = historyAfterTakeover.some(h => h.type === 'host_changed' && h.text.includes('Tab2Peer'));
            if (hasHostChangedEvent) break;
            await sleep(500);
        }
